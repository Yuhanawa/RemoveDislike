...
public static class DirectoryInfoUtils
{
    ...
    public static FileInfo[] TryGetAllFiles(this DirectoryInfo directoryInfo)
    {
        ...
        try
        {
            ...
            foreach (DirectoryInfo directory in directoryInfo.GetDirectories())
                try
                {
                    files.AddRange(directory.TryGetAllFiles());
                }
                catch
                {
                    /* ignore errors */
    ...
    public static string[] TryGetAllFilePaths(this DirectoryInfo directoryInfo)
    {
        ...
        try
        {
            ...
            foreach (DirectoryInfo directory in directoryInfo.GetDirectories())
                try
                {
                    files.AddRange(directory.TryGetAllFilePaths());
                }
                catch
                {
                    /* ignore errors */
    ...
    public static void TryGetAllFilePaths(string path, Action<string> action)
    {
        try
        {
            try
            {
                Directory.GetFiles(path).ToList().ForEach(action);
            }
            catch
            {
                /* ignore errors */
            ...
            foreach (string directory in Directory.GetDirectories(path))
                try
                {
                    TryGetAllFilePaths(directory, action);
                }
                catch
                {
                    /* ignore errors */
